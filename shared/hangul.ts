 ...
 *
 * v1 Basics inventory uses a non-tensed subset; full Unicode tables are
 * included so content answers and future modules remain valid.
 */

/** Compatibility jamo → choseong (L) index 0–18. */
...
/** Compatibility jamo → jungseong (V) index 0–20. */
...
/** Full Unicode jongseong table (28 entries including empty). */
...
/** v1 Basics plain consonants (14). */
export const V1_CONSONANTS = [
  "ㄱ",
  "ㄴ",
  "ㄷ",
  "ㄹ",
...
/** v1 Basics basic vowels (10). */
export const V1_VOWELS = ["ㅏ", "ㅑ", "ㅓ", "ㅕ", "ㅗ", "ㅛ", "ㅜ", "ㅠ", "ㅡ", "ㅣ"] as const;

/** v1 Basics batchim subset (7). Indexes: ㄱ=1, ㄴ=4, ㄷ=7, ㄹ=8, ㅁ=16, ㅂ=17, ㅇ=21. */
...
/**
 * Decompose a single precomposed Hangul syllable into L/V/T jamo.
 * T is "" when there is no final consonant.
 * @throws if the character is not a precomposed Hangul syllable
 */
export function decomposeHangul(syllable: string): HangulParts {
  if (syllable.length !== 1) {
    throw new Error(`Expected single syllable, got ${JSON.stringify(syllable)}`);
  }
