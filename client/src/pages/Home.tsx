...
import { ArrowRight, BookOpenText, Bot, CheckCircle2, GraduationCap, Headphones, ShieldCheck, Sparkles, Volume2 } from "lucide-react";
    ...
    <section className="relative overflow-hidden bg-[var(--cream)]">
      ...
      <div className="container relative grid min-h-[680px] items-center gap-12 py-16 lg:grid-cols-[1.08fr_.92fr] lg:py-24">
        <div className="max-w-3xl">
          ...
          <div className="mt-9 flex flex-wrap gap-3"><Link href={`/lesson/${overview.completedLessons ? nextChapter : 1}`}><Button className="h-13 rounded-full bg-[var(--navy)] px-7 text-base text-white shadow-lg shadow-[var(--navy)]/15 hover:bg-[var(--navy)]/92">{overview.completedLessons ? "পড়া চালিয়ে যান" : "বিনামূল্যে শুরু করুন"}<ArrowRight className="size-4" /></Button></Link><Link href="/curriculum"><Button variant="outline" className="h-13 rounded-full border-[var(--navy)]/20 bg-white/55 px-7 text-base text-[var(--navy)]">পাঠ্যক্রম দেখুন</Button></Link></div>
          <div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-[var(--navy)]/55"><span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-[var(--sage)]" />Bangla-first lessons</span><span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-[var(--sage)]" />Guest progress saved</span><span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-[var(--sage)]" />Mobile friendly</span></div>
        </div>
