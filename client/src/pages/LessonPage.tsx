...
import { enrolVocabulary, recordVocabResult, recordWeakAttempt } from "@/lib/srs";
import { trpc } from "@/lib/trpc";
import { useAuth } from "@/_core/hooks/useAuth";
import { isBasicsComplete } from "@shared/basics";
import type { EpsQuestion, Lesson, PracticeQuestion } from "@shared/lesson";
import { harvestVocabResults } from "@shared/vocabReview";
...
export default function LessonPage() {
  ...
  const saveProgress = (patch: Parameters<typeof updateChapterProgress>[1], minutes = 5) => {
    ...
    if (isAuthenticated) saveRemote.mutate({ chapter, ...patch, minutes, tzOffsetMinutes: tzOffsetMinutes() });
    return saved;
  };
  const record = (
    ...
    // Per-word signal: a missed question is the only evidence the app gets about
    // which vocabulary is actually shaky, so harvest it instead of asking the
    // learner to self-rate.
    if (lesson) {
      for (const hit of harvestVocabResults(lesson, payload?.answers ?? {})) {
        recordVocabResult({ chapter, word: hit.word, glossBn: hit.glossBn, correct: hit.correct });
      }
    }
  ...
  return <>
    ...
    <div className="container py-8 md:py-12">
      ...
      {active === "vocabulary" && <VocabularyView lesson={lesson} done={local?.vocabDone} onDone={() => { saveProgress({ vocabDone: true }); enrolVocabulary(chapter, lesson.vocabulary); }} />}
