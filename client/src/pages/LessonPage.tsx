...
export default function LessonPage() {
  ...
  const record = (
    ...
    // The chapter entry above still feeds the dashboard, planner and readiness surfaces. This
    // schedules the individual words and questions that were actually missed, so a learner who
    // misses 3 of 30 reviews those 3 instead of re-reading the chapter.
    if (lesson && payload) {
      const { correctIds } = scoreLessonExam(lesson, kind, payload.answers, payload.matching ?? {});
      ...
      recordItemReviews(
        kind === "practice"
          ? {
              kind: "practice",
              chapter,
              // Matching questions need a pairing UI the review drill does not have.
              results: lesson.practice
                .filter(question => question.type !== "matching")
                .map(question => ({
                  itemId: question.id,
                  labelBn: question.questionBn,
                  correct: correct.has(question.id),
                })),
            }
          : {
              kind: "eps",
              chapter,
              results: lesson.epsQuestions.map(question => ({
                itemId: question.id,
                labelBn: question.questionBn,
                correct: correct.has(question.id),
              })),
  ...
  return <>
    ...
    <div className="container py-8 md:py-12">
      ...
      {active === "vocabulary" && <VocabularyView lesson={lesson} done={local?.vocabDone} onDone={() => { saveProgress({ vocabDone: true }); if (!writesBlocked) seedVocabReviews({ chapter, entries: lesson.vocabulary.map(word => ({ itemId: word.ko, labelBn: `${word.ko} · ${word.bn}` })) }); }} />}
