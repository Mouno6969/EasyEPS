...
export function MicroSessionPanel({
  ...
  const fallbackSessions = [
    { minutes: 5, titleBn: hangulReady ? `অধ্যায় ${nextChapter} · শব্দ ঝালাই` : "হ্যাঙ্গুল ঝালাই", detailBn: "একটি ছোট flashcard ও উচ্চারণ সেশন", href: hangulReady ? `/lesson/${nextChapter}` : "/basics" },
    { minutes: 7, titleBn: review?.labelBn ?? "দুর্বল জায়গা রিভিউ", detailBn: review ? `আজ নির্ধারিত · mastery ${review.mastery}%` : "নতুন ভুল জমলে এখানে ব্যক্তিগত রিভিউ আসবে", href: review ? hrefForReview(review) : hangulReady ? `/lesson/${nextChapter}` : "/basics" },
    { minutes: 12, titleBn: "১০ প্রশ্নের স্মার্ট টেস্ট", detailBn: "দুর্বল অধ্যায়কে অগ্রাধিকার দিয়ে দ্রুত পরীক্ষা", href: "/mock-test?count=10&mode=smart" },
  ];
  ...
  return (
    <section className="paper-card mt-7 p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {sessions.map(({ minutes, titleBn, detailBn, href }, index) => {
          const Icon = index === 0 ? RotateCcw : index === 1 ? BookOpenText : BrainCircuit;
          return <Link key={`${href}-${minutes}`} href={href} className="group rounded-2xl border border-[var(--navy)]/10 bg-[var(--cream)] p-4 transition hover:border-[var(--gold)]/45 hover:bg-white">
            ...
            <h3 className="mt-4 font-bold text-[var(--navy)]">{titleBn}</h3>
            <p className="mt-2 text-xs leading-5 text-[var(--navy)]/50">{detailBn}</p>
