    ...
    <section className="paper-card mt-7 overflow-hidden">
      <div className="grid lg:grid-cols-[.82fr_1.18fr]">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2"><div className="rounded-2xl border border-[var(--navy)]/8 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--navy)]/42">Delayed retention</p><p className="mt-1 font-serif text-xl font-bold text-[var(--navy)]">{report.metrics.retentionAccuracy || "—"}{report.metrics.retentionAccuracy ? "%" : ""}</p></div><div className="rounded-2xl border border-[var(--navy)]/8 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--navy)]/42">Listening</p><p className="mt-1 font-serif text-xl font-bold text-[var(--navy)]">{report.metrics.listeningAccuracy || "—"}{report.metrics.listeningAccuracy ? "%" : ""}</p></div><div className="rounded-2xl border border-[var(--navy)]/8 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--navy)]/42">Confidence match</p><p className="mt-1 font-serif text-xl font-bold text-[var(--navy)]">{report.metrics.confidenceCalibration || "—"}{report.metrics.confidenceCalibration ? "%" : ""}</p></div><div className="rounded-2xl border border-[var(--navy)]/8 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--navy)]/42">First exposure</p><p className="mt-1 font-serif text-xl font-bold text-[var(--navy)]">{report.metrics.novelAccuracy || "—"}{report.metrics.novelAccuracy ? "%" : ""}</p></div></div><p className="mt-4 text-[10px] leading-4 text-[var(--navy)]/38">Readiness একটি সহায়ক সূচক; এটি recent performance, delayed retention, curriculum coverage, scheduled-review mastery ও study consistency মিলিয়ে হিসাব করা হয়—আনুষ্ঠানিক EPS-TOPIK ফল নয়।</p>
        </div>
      </div>
    </section>
  );
