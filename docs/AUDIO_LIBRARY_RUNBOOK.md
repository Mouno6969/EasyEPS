...
The current shipped generated release contains **one full-dialogue WAV for every one of the 180 authored dialogues across all 60 lessons**, **12 individual lesson-1 dialogue-line WAVs**, and **12 separate deliberately slow pronunciation-reference WAVs** for the earlier lesson-1 dialogue pronunciation feature. Separately, the Basics module now contains **185 unique slow pronunciation WAVs** reused across **621 exact character, speak-lab, read-word, and listen-choice references**. The full-dialogue recordings are intended for natural full-conversation replay. Individual line replay uses the normal lesson line clips where present and otherwise retains speaker-aware browser TTS and pitch fallback. The pronunciation coach and Basics pronunciation controls default to deliberately slow delivery and offer normal 1× playback. EPS listening-question passages outside Basics are not represented as attached generated clips in this release; they continue to use their authored passage with the existing dialogue/browser-TTS fallback.
...
Generated public WAV files are checked as mono 24 kHz PCM16 audio. The Basics attachment script computes SHA-256 and duration metadata from the canonical public file and writes the same clip reference into every matching Basics object and `content/audio/manifest.json`. The manifest must never be rebuilt by a script that clears existing clips; attachment scripts must merge into the existing inventory.
...
The current generated dialogue convention uses `Iapetus` for the first two-speaker voice position and `Erinome` for the second. The Basics pronunciation library uses the stable synthetic `Iapetus` voice for clear, deliberately slow beginner drills. Multi-speaker assembly additionally uses stable distinct voices such as `Algieba` and `Sulafat` for later positions. These are technical voice assignments, not claims about human performers.
...
For a full listening question, attach one generated clip to the question’s `audio` field only when the exact passage has actually been rendered, validated, and attached. Do not attach a dialogue recording to an unrelated listening question merely to increase coverage. For a pronunciation coach, attach a separately rendered slow reference to a dialogue line’s `pronunciationAudio` field; keep the ordinary `audio` field unchanged for normal dialogue replay. For Basics, attach the exact source-text clip to the jamo item, speak item, read item, or listen-choice question that owns that source text; reuse is content-based, not question duplication. Until listening clips are generated and attached, preserve the authored passage and browser fallback.

## Runtime behavior

`GuidedListening`, dialogue playback, and pronunciation coaching prefer a usable attached clip. Generated clips report `generated-audio`; human-reviewed clips report `reviewed-audio`; browser fallback reports `browser-tts` or the explicitly documented pitch fallback. If an attached file fails, the application continues with Korean browser TTS. Full-dialogue replay prefers the dialogue-level recording before falling back to line-by-line speaker-aware playback. Individual line replay remains line-specific where a line clip exists and otherwise uses browser TTS. Pronunciation coaching and Basics pronunciation exercises start with the deliberately slow reference and expose both slow 0.6× and normal 1× controls; the chosen rate is applied to either the generated WAV or browser TTS. Basics clips live under `/audio/generated/basics-pronunciation-slow/` and remain explicitly AI-generated.
